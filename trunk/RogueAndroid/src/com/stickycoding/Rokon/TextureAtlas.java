...
/**
 * @author Richard
 * Holds a collection of Texture's, organized so they can be stored as one texture on the hardware
 */
public class TextureAtlas {
	 ...
	 * 
	 * @param i
	 * @return the Texture at the specified Index i, NULL if empty or incorrect index
	 */
	public Texture textureAtIndex(int i) {
		if (i > _texture.length) {
			Debug.warning("Index out of bounds " + i);
			return null;
		}
		if (_texture[i] == null)
		{
			Debug.warning("No texture at index " + i);
		}
		return _texture[i];
	}
}
